"""
This Class is responsible for storing all information, determining valid moves, and move log
"""
...
class GameState():
    ...
    def getValidMoves(self):
        ...
        for i in range(len(moves)-1, -1, -1):
            self.makeMove(moves[i]) #changes sides
        #3 generate all opponents moves
        #4 for each of your opponent's moves, see if they attack king
        #5 if they do attack, not valid move
            self.whiteToMove = not self.whiteToMove #change back to the right side
            if self.inCheck():
